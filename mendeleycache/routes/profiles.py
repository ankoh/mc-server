...
class ProfilesController:
    ...
    def get_profile_page(self):
        ...
        :return:
        """
        log.info('The route GET /profile/ has been triggered')

        if 'first_name' in request.args:
            first_name = request.args['first_name']
            log.debug('Query parameter "first_name" = %s' % first_name)
        else:
            return "You need to provide the query parameter first_name", 400

        if 'last_name' in request.args:
            last_name = request.args['last_name']
            log.debug('Query paramter "last_name" = %s' % last_name)
        else:
            return "You need to provide the query parameter last_name", 400
